...
== Testing
For now tests run integrated with a local running instance of GeoServer. 
Download version 2.1.3 binary from {here}[http://sourceforge.net/project/downloading.php?groupname=geoserver&filename=geoserver-2.1.3-bin.zip&use_mirror=softlayer], unzip, enter the directory and run:
  > java -jar start.jar

  Go to rgeoserver source directory and execute the tests:
  > rake 
...
- <b>v0.1.0</b> - Initial alpha release

== TODO
- Complete stores and coverages functionality, handle Layers, styles and data upload.
- Complete documentaion and test coverage with embedded Jetty.
